...
New exports use action schema v0.3. Each entry retains its current disposition and note for convenient reading and an append-only `events` chain for disposition changes, note revisions, imports, and reversals. Current state must equal replayed event state. Events have UUIDs, timestamps, origins, and parent links where applicable; changing a note or undoing a choice appends an event rather than erasing history. v0.1/v0.2 imports remain supported and are normalized without pretending they contained event history.
...
1. Run `python3 scripts/validate_review_actions.py <review-actions.json>` (or the equivalent absolute path), which validates the schema, unique IDs, history order, and timestamps.
...
For v0.3, also validate event UUID uniqueness, parent existence, chronological order, replayed current state, and maximum sizes. Never trust a client-supplied current disposition when it disagrees with the event chain.
