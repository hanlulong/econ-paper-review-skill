...
When a plugin manifest is present but the managed-runtime descriptor is absent, remember that marketplace installation is skill-only. If the core dependency or PDF doctor fails, say exactly what is missing and explain that the repository's [complete setup](https://github.com/hanlulong/econ-paper-review-skill/blob/main/docs/INSTALL.md) prepares the Python runtime and Review Desk while Poppler remains an explicit user choice. Do not claim the plugin install made those components ready, and do not install system software without the user's authorization.
